...
PORT=8795
OUT="$(dirname "$0")/redundant4_results"
mkdir -p "$OUT"
...
for k in range(6):                                                      # 4 unrelated launches
    open(f"{out}/other{k}.txt","w").write(f"OTHER{k}. " + blk(f"OTH{k}", 120, 9000+k*1000))
PY
...
# -np 4 so one slot can stay idle while the other works
"$BIN" -m "$MODEL" --port "$PORT" -c 16384 -np 4 -ngl 99 -cram 8192 -lv 4 --no-warmup \
  > "$OUT/server.log" 2>&1 &
PID=$!
...
req "$OUT/park.txt"
echo "2) six unrelated requests, each a fresh task launch:"
for k in 0 1 2 3 4 5; do req "$OUT/other$k.txt"; done
