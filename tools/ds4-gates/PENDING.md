...
> logical bytes ⇒ ggml_mem_ranges never tracks it ⇒ a prior op physically overlapping the
> scratch races the mask fill under the concurrent encoder. Ubatch changes graph layout, so
> a misaligned ubatch lands scratch on a live region. FIX: one unconditional
> `ggml_metal_encoder_memory_barrier(enc)` before the mask-fill dispatch (ggml-metal-ops.cpp,
> commit 4b62ac77b on origin/ds4-ports). PROOF: 3-arm gate n=8 (ub255 3/8 flip
> concurrency-ON; 8/8 CONCURRENCY_DISABLE=1; ub256 8/8 either way) → race; fix 16/16 identical
> both ubatch; multi-chunk prefill byte-identical to static both ubatch; harness ALL PASSED.
> The dense/tiled `ctx_lens` mask edits were a RED HERRING — reverted, never needed.
>
> **Flag-flip: DSV4 already DEFAULT under `--kv-paged`** (`e2d787613`, 2026-08-12). The
> owner asked 2026-08-14 "if the models list is solved, then sure." Verdict: the 15 verified
> standard/dense archs on the named list page by default today (no extra env). Remaining env
> gates stay: `DS4P_PAGED_HYBRID` (qwen3next / inkling / qwen35 — graph exists, but flipping
> would also page unverified hybrid siblings), `DS4P_PAGED_SWA` (laguna/mimo2/step35/gemma3 —
> same class), `DS4P_PAGED_MSA` (minimax-m3 — store+gather bit-exact, real-model verify PARKED).
> I will NOT flip HYBRID/SWA/MSA globally: that is the 2026-08-09 silent-static trap applied
> to a family, not one arch. `--kv-paged` itself stays opt-in (`kv_paged=false` in common.h).
>
> ⚠ The long "defect #2" trail below (ub256 multi-chunk inp_pos clobber) is **STALE** — that
> was fixed 8-12 via the gallocr missing-leaf-check (commit 570842ef8). Not an open bug.
...
✅ **LANDED 2026-08-13 — n_batch==n_ubatch is a designed refusal, not an assert.**
Three sites, one message: `kv_paged requires n_batch == n_ubatch (got n_batch=N n_ubatch=M). Pass -b N -ub N with the same N.`
- FIRST WALL: `llama_context` ctor throws before `create_memory` (covers hybrid / SWA / DSV4 / MSA; a
  silent `return nullptr` from `create_memory` would look like "encoder, no KV" and keep serving).
- SECOND WALL: `llama-model.cpp` dense paged path throws the same text (never falls through to
  `new llama_kv_cache` — that is the 2026-08-09 silent-static trap).
- SECOND WALL: `llama_paged_scheduler_init` logs + returns null (server already refuses on
  `!paged_sched`).
SMOKED on stories260K, rebuilt llama-cli at ac9594ad4 + this change:
- `-b 512 -ub 256 --kv-paged` → rc=1, named both values, 0 GGML_ASSERT / 0 abort.
- `-b 512 -ub 512 --kv-paged` → model loaded, `paged serving: scheduler initialized`, DS4P-CHECKOUT,
  request answered. On origin/ds4-ports as `08f85dbfc` (pushed 2026-08-14). Did not convert the sibling `kv_unified` / KV-sharing asserts.
...
| item | state |
|---|---|
| ✅ **CLASS SWEEP: discarded-VOID across the gate suite (2026-08-13)** — 2 genuine holes FOUND + FIXED | Fan-out audit of the correct-producer-no-consumer class (calibrated on warmup=hole vs arm=covered) over the priority no-`set -e` gates. **HOLE 1 `multislot_gate.sh`:** `gate_assert_paged_consumed`'s rc=1 (LAW-6 "ZERO layers consumed") was read via PIPESTATUS then used only for a cosmetic note — no `fails` increment — so a paged arm silently fallen back to static gave correct static text → `paged:CLEAN` → **"PASS both paths clean" + exit 0**, the exact 4.5h lesson the check exists to prevent. FIXED: mirrors `long_context_gate.sh:168` (`fails+1`), the no-paged-log UNVERIFIED branch also fails now, and the final PASS line is gated on `fails==0` (prints VOID instead — the last line is the one a reader quotes). **HOLE 2 `consumer_taint_gate.sh`:** run()'s tee'd "NEVER READY" sentinel is CAPTURED by `\| tail -1` as the arm's "sha" while the pipe discards its return 1; guard was also asymmetric (checked `$a` for NO_LOGPROBS, never `$b`). Three false-CONSUMED shapes → all normalized to empty → existing `-z` INCONCLUSIVE guard consumes them. Both `bash -n` OK; verdict logic proven on 9 simulated scenarios (3 former false verdicts corrected, all healthy paths unchanged). **15 priority gates verified COVERED** (long_context, arch_serve, warm_multislot, default_config, champ_parity32k, champ_tailfill, batch_offset, eviction, champ probes, metal walls, paged baseline). Known-not-fixed: `hybrid_decode_gate.sh` dead-arm case yields a false FAIL (outside the class, always exits 0, tautology warning adjacent) — cosmetic. NOT swept: non-priority gates (hybrid_paged, quant_kv_e2e, state_serdes, prefix_share, draft_pair, paged_multimodel, p15/p28, quench, reval) — second pass available. |
| ⚡ **`arm` VOIDs are discarded too — but correctness is COVERED (fail-FAST opportunity, not a hole)** | Found 2026-08-13 auditing for more of the warmup class. `arm()` has 5 fatal `return 1` VOIDs (179 server-never-up · 211 stale-binary · **223 static built a pool · 224 paged shows no pool · 277 pool nobody consumed** — the 4.5h lesson). The ABBA call sites are `arm X; mv …` (bare `;`), and the gate is `set -uo pipefail` (no `-e`), so `arm`'s return is DISCARDED — same shape as the warmup bug. **BUT verified NOT a correctness hole:** `.res` is written only at the END of `arm()` (line 354), AFTER every VOID, so a VOIDed arm writes no `.res`; `.res` are cleared at start (line 99) so nothing stale survives; and the summariser does `json.load(open(f"{D}/*.res"))` inside a try that prints "VOID: ABBA incomplete" on any missing file (566-570). So a discarded `arm` VOID → missing `.res` → downstream VOID. **The only cost is TIME:** the gate runs the remaining arms (up to ~10h more on a 14h 256k ABBA) then prints a vaguer "ABBA incomplete" instead of failing at minute 20 with the precise cause already printed. Fail-fast fix = make `arm`'s 5 VOIDs `exit 1` (EXIT trap at 112 cleans up) OR wrap the 6 call sites `arm X \|\| exit 1`. NOT done mid-marathon: 5 fatal paths inside the bar instrument each need call-context verification, and the reward is operational (cycle-time) not correctness. Flagged for a deliberate pass. Aligns with the accelerate-cycle-time principle if the owner wants it. |
...
| ~~warm-up response is never checked~~ | ✅ **CLOSED 2026-08-13.** Two fixes: (1) the warm-up completion is no longer curled to `/dev/null` — it captures `http_code` + parses `content` length and VOIDs on non-200/empty (the "response body still unread" half). (2) **The load-bearing fix found while wiring it:** the gate is `set -uo pipefail` (NO `-e`) and called `warmup` BARE, so warmup's `return 1` VOID — Edit C's log-grep AND the new response check — was DISCARDED and the gate measured anyway, reporting the cold-arm confound as an EFFECT. Now `warmup || { …; exit 1; }` consumes it. Correct-producer-no-consumer, in the prelude that exists to protect the bar number. The "never came up" path still `continue`s → returns 0 → runs cold with a warning (optional-prelude intent preserved; semantics proven by scratchpad/warmup_semantics_test.sh). bash -n OK. NOT yet exercised by a live ABBA. |
...
| lint sweep | **RUN 2026-08-13, and the sweep found a defect in a LINT:** `lint_paged_consumers` PASS · `lint_scrub_coverage` PASS · `lint_common_laws` 9/47 reach · `lint_vacuous_pass` 10/37 vocabulary-level. **`lint_claimed_vs_entered` FIXED three times:** (1) line-based grep missed `\`-continued concurrent curls; (2) comments containing `-np 2` were scored as a CLAIM; (3) Grok #9808: `-np ${VAR:-N}` defaults were INVISIBLE -- hid `batch_offset_invariant_gate` and `warm_multislot_gate`, the two best-guarded multi-slot gates. Now extracts N from `:-N`. Full lint: 9 rows, EXIT 0. `hybrid_paged_gate` dropped `-np 2` to `-np 1` (op-level bar, not multi-slot) and combined its two EXIT traps so the privacy scrubber actually runs. |
