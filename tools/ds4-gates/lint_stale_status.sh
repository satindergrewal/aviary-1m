...
echo "  that is already enabled). That shape needs a reader, not a grep.   [shape 4]"
echo "  ⚠⚠ NOR CAN IT SEE CROSS-DOCUMENT DRIFT, and that shape has now bitten.   [shape 5]"
echo "     2026-08-10: PENDING.md called the -np>1 defect OPEN, 27h after it was closed. The"
echo "     resolution was in ANOTHER file's HEAD (FINDINGS-2026-08-06.md:11, 'CLOSED BY"
echo "     MEASUREMENT'); the open claim came from copying that same file's BODY. Head-vs-tail"
echo "     WITHIN one file is exactly inverted from this, so the heuristic scores it 0 and is"
echo "     right to. A cross-document version needs to resolve which doc OWNS a status --"
echo "     semantics, not vocabulary. The parser tried on the coverage sweep misread 4 of 5, and"
echo "     a fragile lint is worse than an honest table, so this is NAMED and not built."
echo "     ⇒ The reader's rule: a defect's status is owned by its FINDINGS file. Cite it, do not"
echo "       restate it -- a copied status cannot be invalidated by the thing that closes it."
exit 0
