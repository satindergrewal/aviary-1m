...
Following Satinder's process: **thought experiments and planning first, kill the bugs and the
...
## 3. THOUGHT EXPERIMENT 2 — does the SLO scheduler help *Satinder's* workload? **Mostly no.**
...
At **`-np 1`** — Satinder's configuration, and *required* for his 1M Ornith runs (otherwise the
