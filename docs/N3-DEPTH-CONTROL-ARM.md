...
```bash
# server
cd /mnt/nvme0/llama.cpp-kt
CUDA_VISIBLE_DEVICES=1 ./build-cuda/bin/llama-server \
  -m /mnt/nvme0/ktdev/Qwen27B-IQ1_KT.gguf -ngl 99 -c 65536 -fa on --port 8091

# harness (note: build/ in that tree is CPU-only, build-cuda/ is the one you want)
/mnt/nvme0/ktdev/n3_depth_control.sh
```

Raw rows: `/mnt/nvme0/ktdev/n3_results/`.
