...
**Status 2026-08-02: PARKED on the owner's word — resume from this doc.**
...
- **Frozen LLM:** DeepSeek-V4-Flash, our served `UD-Q4_K_XL` GGUF (5 shards,
  `<BOX>/bigmodels/dsv4-flash/`). Training runs AGAINST THE QUANT WE SERVE — no
...
| Asset | Where | Notes |
|---|---|---|
| V4-Flash UD-Q4_K_XL (served quant) | box `<BOX>/bigmodels/dsv4-flash/` | 5 shards, shard 1 metadata-only |
...
| Qwen3-VL-4B captioner (dataset engine) | box `<BOX>/models/Qwen3-VL-4B-Instruct-Q8_0.gguf` + `Qwen3-VL-4B-mmproj-BF16.gguf` | Unsloth stock; serves via `PROJECTOR_TYPE_QWEN3VL` already in our trees |
| His Huihui Qwen3-VL-4B abliterated FP8 | box `<BOX>/models/text_encoders/` | vision tower INTACT (315 tensors); convert later if wanted — our tree's converter lacks qwen3_vl (runtime HAS it); upstream converter pull needed |
| Qwen3-4B pilot base | box `<BOX>/models/text_encoders/Qwen3-4B-UD-Q6_K_XL.gguf` (GGUF) + HF safetensors to fetch for BF16 arm | dense, 8 GB BF16 |
| GGUF parse/dequant code | box: `gguf_ablate.py`, `dsv4_build_minitarget.py` | proven Q8_0/Q4 dequant reads — the engine's front half |
| DeepSpec trainer venv | box `<BOX>/DeepSpec/.venv` | torch present (DSpark head training runs there) |
...
**Storage rule (his correction 2026-08-02):** ALL box file work via `ssh user@10.0.x.x`
on `<BOX>|<BOX>|<BOX>`. `/Volumes/8tb` on the Mac is only an SMB mount — never use it
for pipeline I/O.
...
- Serves Qwen3-VL-4B on the box (CPU or GPU-window; NOT against the owner's V4 sessions).
...
- Only if Arm B fails AND the owner approves the spend (RunPod rules: gate-on-binary,
  create-to-delete).
...
- [ ] Dataset builder implemented (box, `<BOX>/vision-graft/`)
...
- [ ] Stage 2 go/no-go with the owner (incl. RunPod fallback decision + video track activation)
