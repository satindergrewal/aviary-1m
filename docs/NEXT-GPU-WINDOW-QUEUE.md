...
**REMAINING for a future window:** Q5 quench-econ (needs a full-box window; fit math banked) · ★ P2-8 ARM2: Mac FULL PASS at 99ea0134 (evict+recompute live, 3-fix chain in results/p28-arm2-mac-PASS-*); remaining = CUDA re-gate same bar + swap-out wall (--n-cpu-blocks 128) · tiled prefill kernel (M7). fork-residual CLOSED — fork acquitted bit-exact (7df46f0).
