...
**Status: DRAFT v2 for Satinder's review. NO implementation until he says go (restated by him 2026-08-02).**
Author: Fable-DSpark | Date: 2026-08-02 | Work surface (exists, untouched): branch `ds4-ports` off
`dsv4-kvpaged` @ `3376ebbc8`, worktree `/mnt/nvme0/wt-ds4-ports` (box).
...
multi-agent serving problems Satinder hit (static slots, prefill stalls, dead clients burning
...
- **Our tree:** kv-paged gives us block-level KV management; fork = clone the block table for the
  ...
  sharing a 100K system+task prefix" — Satinder's actual workload.
...
- RTX PRO 6000 Blackwell 96 GB, q2 (81 GiB, single card): 85.2 t/s prefill(short) / **53.3 t/s decode**.
  ...
  (M3 Max 128 GB q2: 58.5/26.7 short) explains why Satinder rejected the Mac lane (his call stands:
...
- Work only in `/mnt/nvme0/wt-ds4-ports` (branch `ds4-ports`). No `fleet` pushes — per-push
...
- V4 production server (`dsv4-srv` :8331) is Satinder's until he declares V4 testing done; port
