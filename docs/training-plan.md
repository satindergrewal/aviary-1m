...
### Track 1 (ACTIVE, the owner's decision): agent-trace SFT

SFT on vetted permissive trace datasets from the shortlist (the Apache-2.0 25k cleaned set first; the MIT 2M set after provenance verification), plus, over time, traces from the owner's own Claude Code sessions. Teaches tool-call discipline, terse output, Claude-Code-shaped agentic workflows. This is the training work happening now.
