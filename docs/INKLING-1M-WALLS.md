...
unsloth MXFP4_MOE GGUF, 148 GiB, `/mnt/nvme0/bigmodels/inkling-small-gguf/`.
...
|---|---|---|---|---|
| A1 | No inkling support in 0.25.1 | grep of vllm25-venv = 0 hits | support landed in 0.26.0 | new venv `/mnt/data/vllm26-venv` (built) |
...
| B3 | **kv-paged doesn't page for inkling** | paged path allocated FULL -c KV statically: 48 GiB @4×1M (= 4,194,304 × 12 KiB exactly) vs fitter pool estimate 19.5 GiB → OOM; `-b/-ub` changed nothing | **ROOT CAUSE FOUND (Fable-DS4, DESIGN-DS4-P0): `create_memory`'s kv_paged branch sits in the NON-HYBRID leg — inkling never reaches the paged constructor; `--kv-paged` is SILENTLY IGNORED for hybrids = the measured 48 GiB static wall.** Plus the fitter budgets all-layers uniformly | **★ FIXED on ds4-ports, MEASURED CLOSE `071b124e` (2026-08-04): hybrid paged sub-cache + 4d continuous-batching scheduler + paged banded kernel + COW fork. ⚠ the first "decode gate PASS" (96a25014) was INFERRED AND WRONG — serving silently ran static; the op-level gate then surfaced FIVE defects (silent static fallback, unconsumed-input crash, invisible op enum, stale seq state, inherited n_decoded), all fixed. TRUE close: PAGED_ATTN ×144 in serve graphs, static-vs-paged parity 4/4, concurrent 300 steps@running=2 (tools/ds4-gates/hybrid_paged_gate.sh + DESIGN-3B epilogue). Remaining: CUDA arm on the box (never executed) + fleet merge (his word) |
...
1. **B4 gate-widening — ★ DONE on ds4-ports 2026-08-03 (arcs 1-3 both backends: per-stream multi-seq banding 9.7× buffer cut, q8-K banded kernel, analytic mask = zero plane; FORK-DELTA row)**: multi-seq banding + q8-capable
   banded kernel — NOT "build chunking", the mechanism exists (DS4 #1691). ~2–5 days, core.
2. **B3 kv-paged hybrid-arch support — ★ DONE on ds4-ports 2026-08-04 (`071b124e`, see B3 row; CUDA witness owed)** — plan item 3b
