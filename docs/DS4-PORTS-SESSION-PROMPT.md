...
> You are taking over the ds4-ports lane for the owner's llama.cpp fork. Everything you need is
...
- Branch `ds4-ports` off `dsv4-kvpaged` @ `3376ebbc8`, worktree `<BOX>/wt-ds4-ports` (box).
  It exists and is untouched — you start from clean.
- NOTHING touches `fleet` without the owner's explicit per-push approval. Every feature behind an
...
- Kill by exact PID only. His crypto stack (geth/lighthouse/bitcoind/Fulcrum) is untouchable.
  `<BOX>/397b-*` dirs are untouchable. No public posting of anything.
...
- The box: ssh user@10.0.x.x; 2× RTX PRO 6000 Blackwell (95.3 GiB usable each);
  nvcc at /usr/local/cuda-12.9/bin (system 12.0 FAILS compute_120a); fleet tree at
  <BOX>/llama.cpp-fleet; builds go in worktrees, never in the fleet checkout.
- V4-Flash UD-Q4_K_XL at <BOX>/bigmodels/dsv4-flash/; Inkling-Small MXFP4 at
  <BOX>/bigmodels/inkling-small-gguf/. Both serve as your test models.
